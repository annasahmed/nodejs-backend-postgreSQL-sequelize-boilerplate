...
// Schedule update rate daily
cron.schedule('1 0 * * *', async () => {
	await updateCurrenctRates();
});
...
async function updateCurrenctRates(req, res) {
	const currencies = await getCurrencies(req, res, ['country']);

	for (const currency of currencies) {
		const response = await axios.get(API_URL);
		const rate = response?.data?.rates[`${currency.country}`];
		if (!rate) {
			throw new ApiError(
				httpStatus.CONFLICT,
				'Exchange rate not found for this country',
		...
		await db.currency.update(
			{ rate },
			{
				where: { country: currency.country },
			},
		);
		// .then((resultEntity) => resultEntity.get({ plain: true }));
	}
...
async function deleteCurrencyById(req) {
	const deletedCurrency = await db.currency.destroy({
		where: { id: req.params.currencyId || req.body.id },
	});
...
async function updateCurrency(req) {
	...
	const updatedCurrency = await db.currency
		.update(
			{ ...req.body },
			{
				where: { id: req.params.currencyId || req.body.id },
